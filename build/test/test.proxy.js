...
describe('proxy', () => {
    ...
    it('should route requests through http_proxy', async () => {
        ...
        assert.ok(proxiedHosts.length > 0, 'At least one request should have gone through the proxy');
    });
    it('should route requests through HTTP_PROXY', async () => {
        vi.stubEnv('HTTP_PROXY', proxyUrl);
        const results = await check({ path: targetUrl });
    ...
    it('should bypass proxy when no proxy env vars are set', async () => {
        ...
        const results = await check({ path: targetUrl });
        assert.ok(results.passed);
        assert.strictEqual(proxiedHosts.length, 0, 'Proxy should not have been contacted when env vars are unset');
    });
});
