...
describe('fragment identifier validation', () => {
    ...
    it('should validate valid fragment identifiers when checkFragments is enabled', async () => {
        ...
        mockPool
            ...
            .reply(200, '<html><body><div id="valid-section">Content</div></body></html>', { headers: { 'content-type': 'text/html' } });
        ...
        const baseUrlResult = results.links.find((l) => l.url === 'http://example.com/page.html');
    ...
    it('should mark invalid fragment identifiers as broken when checkFragments is enabled', async () => {
        ...
        mockPool
            .intercept({ path: '/page.html', method: 'GET' })
            .reply(200, '<html><body><div id="different-section">Content</div></body></html>', { headers: { 'content-type': 'text/html' } });
        const results = await check({
        ...
        const fragmentResult = results.links.find((l) => l.url === 'http://example.com/page.html#invalid-section');
        expect(fragmentResult?.state).toBe(LinkState.BROKEN);
        expect(fragmentResult?.failureDetails?.[0]).toBeInstanceOf(Error);
        expect((fragmentResult?.failureDetails?.[0]).message).toContain("Fragment identifier '#invalid-section' not found on page");
    });
    it('should not check fragments when checkFragments is disabled', async () => {
        ...
        const fragmentResult = results.links.find((l) => l.url.includes('#invalid-section'));
    ...
    it('should handle multiple fragments pointing to the same page', async () => {
        ...
        mockPool
            .intercept({ path: '/page.html', method: 'GET' })
            .reply(200, '<html><body><div id="section-one">Content</div><div id="section-two">More</div></body></html>', { headers: { 'content-type': 'text/html' } });
        const results = await check({
    ...
    it('should emit events for broken fragments', async () => {
        ...
        const brokenFragmentEvent = linkEvents.find((e) => e.url.includes('#invalid-section') && e.state === LinkState.BROKEN);
    ...
    it('should work with local file server (Node.js Readable streams)', async () => {
        ...
        expect(results.passed).toBe(false);
        // Find the broken fragment link
        const brokenFragment = results.links.find((l) => l.url.includes('#nonexistent-section'));
        expect(brokenFragment?.state).toBe(LinkState.BROKEN);
        expect(brokenFragment?.failureDetails?.[0]).toBeInstanceOf(Error);
        expect((brokenFragment?.failureDetails?.[0]).message).toContain("Fragment identifier '#nonexistent-section' not found on page");
        // Valid fragments should pass
        const _validFragment1 = results.links.find((l) => l.url.includes('#valid-section'));
        const _validFragment2 = results.links.find((l) => l.url.includes('#another-section'));
        ...
        expect(results.links.filter((l) => (l.url.includes('#valid-section') ||
            l.url.includes('#another-section')) &&
            l.state === LinkState.BROKEN)).toHaveLength(0);
    });
    it('should validate fragments in markdown files with GitHub-style heading IDs', async () => {
        ...
        expect(results.links.filter((l) => (l.url.includes('#authoring') || l.url.includes('#examples')) &&
            l.state === LinkState.BROKEN)).toHaveLength(0);
    });
    it('should mark invalid markdown fragments as broken', async () => {
        ...
        expect(results.passed).toBe(false);
        // Find the broken fragment link
        const fragmentResult = results.links.find((l) => l.url.includes('#nonexistent'));
        expect(fragmentResult?.state).toBe(LinkState.BROKEN);
        expect(fragmentResult?.failureDetails?.[0]).toBeInstanceOf(Error);
        expect((fragmentResult?.failureDetails?.[0]).message).toContain("Fragment identifier '#nonexistent' not found on page");
    });
    it('should validate same-page fragment links (issue #770)', async () => {
        ...
        const validFragments = results.links.filter((l) => (l.url.includes('#valid-section') ||
            l.url.includes('#another-section')) &&
    ...
    it('should validate GitHub-style permalink anchors', async () => {
        ...
        const validFragmentLinks = results.links.filter((l) => (l.url.includes('#section-one') || l.url.includes('#section-two')) &&
            l.state === LinkState.BROKEN);
        expect(validFragmentLinks).toHaveLength(0);
        // Invalid fragment should fail
        const brokenFragment = results.links.find((l) => l.url.includes('#nonexistent'));
        expect(brokenFragment?.state).toBe(LinkState.BROKEN);
        expect(brokenFragment?.failureDetails?.[0]).toBeInstanceOf(Error);
        expect((brokenFragment?.failureDetails?.[0]).message).toContain("Fragment identifier '#nonexistent' not found on page");
    });
    describe('validateFragments', () => {
        it('should validate fragments against HTML content', async () => {
            ...
            expect(results.find((r) => r.fragment === 'missing')?.isValid).toBe(false);
        });
