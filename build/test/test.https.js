...
describe('https enforcement', () => {
    ...
    describe('off mode (default)', () => {
        it('should accept both HTTP and HTTPS links', async () => {
            ...
            const httpLink = results.links.find((link) => link.url.includes('/target'));
            assert.ok(httpLink);
    ...
    describe('warn mode', () => {
        it('should accept HTTP links but emit warnings', async () => {
            ...
            const httpLinks = results.links.filter((link) => link.url.startsWith('http://'));
    ...
    describe('error mode', () => {
        it('should treat HTTP links as broken', async () => {
            ...
            for (const link of httpLinks) {
                ...
                const hasHttpsError = link.failureDetails?.some((detail) => detail instanceof Error
            ...
            const httpsLinks = results.links.filter((link) => link.url.startsWith('https://'));
        ...
        it('should not affect HTTPS links', async () => {
            ...
            const mainPage = results.links.find((link) => link.url.includes('/target'));
            assert.ok(mainPage);
    ...
    describe('local file scanning with requireHttps', () => {
        it('should not flag local static server URLs as broken', async () => {
            ...
            const localLink = results.links.find((link) => link.url.endsWith('other.html'));
            assert.ok(localLink, 'Should find the local link to other.html');
            assert.strictEqual(localLink.state, LinkState.OK, 'Local static server links should not be flagged as HTTPS violations');
            // Find the external HTTP link (example.com)
            const externalHttpLink = results.links.find((link) => link.url === 'http://example.com/');
            assert.ok(externalHttpLink, 'Should find the external HTTP link');
            assert.strictEqual(externalHttpLink.state, LinkState.BROKEN, 'External HTTP links should still be flagged when requireHttps is error');
        });
        it('should emit warnings for external HTTP but not local static server', async () => {
            ...
            assert.ok(warnings.some((w) => w.url === 'http://example.com/'), 'Should warn about external HTTP link');
            assert.ok(!warnings.some((w) => w.url.endsWith('other.html')), 'Should not warn about local static server links');
        });
    });
});
