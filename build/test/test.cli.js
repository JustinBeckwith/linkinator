...
describe('cli', () => {
    let server;
    const package_ = JSON.parse(fs.readFileSync(new URL('../package.json', import.meta.url), 'utf8'));
    ...
    it('should have build/package.json with matching version', () => {
        ...
        const buildPkg = JSON.parse(fs.readFileSync(new URL('../build/package.json', import.meta.url), 'utf8'));
        assert.strictEqual(buildPkg.version, rootPkg.version, 'build/package.json version should match root package.json version');
    });
    ...
    it('should not show 200 links if verbosity is ERROR with JSON', async () => {
        ...
        const links = JSON.parse(response.stdout).links;
    ...
    it('should show callstacks for verbosity=DEBUG', async () => {
        const response = await execa(node, [linkinator, 'test/fixtures/basic', '--verbosity', 'DEBUG'], {
            reject: false,
        });
    ...
    it('should fail if a url search is provided without a replacement', async () => {
        const response = await execa(node, [linkinator, '--url-rewrite-search', 'boop', 'test/fixtures/basic'], {
            reject: false,
        });
        assert.strictEqual(response.exitCode, 1);
    ...
    it('should fail if a url replacement is provided without a search', async () => {
        const response = await execa(node, [linkinator, '--url-rewrite-replace', 'beep', 'test/fixtures/basic'], {
            reject: false,
        });
        assert.strictEqual(response.exitCode, 1);
        assert.match(response.stderr, /flag must be used/);
    ...
    it('should warn on retries', async () => {
        ...
        assert.strictEqual(response.exitCode, 0);
        assert.match(response.stdout, new RegExp(`Retrying: http://localhost:${port}/`));
    });
    describe('custom headers', () => {
        ...
        it('should fail on malformed header without colon', async () => {
            const response = await execa(node, [linkinator, 'test/fixtures/basic', '--header', 'InvalidHeader'], { reject: false });
            assert.match(response.stderr, /Invalid header format.*Use.*Header-Name:value/);
        });
        it('should fail on header with empty name', async () => {
            const response = await execa(node, [linkinator, 'test/fixtures/basic', '--header', ':value'], { reject: false });
            assert.match(response.stderr, /Header name cannot be empty/);
        });
        it('should fail on header with empty value', async () => {
            const response = await execa(node, [linkinator, 'test/fixtures/basic', '--header', 'X-Empty:'], { reject: false });
