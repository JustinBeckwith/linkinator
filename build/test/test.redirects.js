...
describe('redirects', () => {
    ...
    it('should follow 307 redirects and mark as OK', async () => {
        ...
        assert.strictEqual(link.state, LinkState.OK, '307 redirect should be marked as OK');
        assert.strictEqual(link.status, 200, 'Should return final status after redirect');
    });
    it('should follow 308 redirects and mark as OK', async () => {
        ...
        assert.strictEqual(results.links.length, 1);
        const link = results.links[0];
        assert.strictEqual(link.state, LinkState.OK, '308 redirect should be marked as OK');
        assert.strictEqual(link.status, 200, 'Should return final status after redirect');
    });
    it('should follow 301 redirects and mark as OK', async () => {
        ...
        assert.strictEqual(results.links.length, 1);
        const link = results.links[0];
        assert.strictEqual(link.state, LinkState.OK, '301 redirect should be marked as OK');
        assert.strictEqual(link.status, 200, 'Should return final status after redirect');
    });
    it('should follow 302 redirects and mark as OK', async () => {
        ...
        assert.strictEqual(results.links.length, 1);
        const link = results.links[0];
        assert.strictEqual(link.state, LinkState.OK, '302 redirect should be marked as OK');
        assert.strictEqual(link.status, 200, 'Should return final status after redirect');
    });
    it('should follow redirects when checking links in crawled pages', async () => {
        ...
        const redirectLinks = results.links.filter((link) => link.url.includes('redirect-'));
        assert.strictEqual(redirectLinks.length, 4);
        // All redirect links should be marked as OK
        for (const link of redirectLinks) {
            assert.strictEqual(link.state, LinkState.OK, `Link ${link.url} should be OK`);
            assert.strictEqual(link.status, 200, `Link ${link.url} should have status 200 after following redirect`);
        }
    });
    describe('redirect modes', () => {
        ...
        it('should emit warnings in warn mode', async () => {
            ...
            assert.strictEqual(results.links[0].state, LinkState.OK);
            assert.strictEqual(results.links[0].status, 200);
            assert.strictEqual(redirectWarnings.length, 1, 'Should emit one redirect warning');
        ...
        it('should mark redirects as broken in error mode', async () => {
            ...
            assert.ok(results.links[0].failureDetails &&
                results.links[0].failureDetails.length > 0, 'Should have failure details');
        });
        it('should handle non-standard redirects in allow mode', async () => {
            ...
            const addr = nonStandardServer.address();
        ...
        it('should warn about non-standard redirects in warn mode', async () => {
            ...
            try {
                ...
                assert.strictEqual(redirectWarnings[0].isNonStandard, true, 'Should detect non-standard redirect');
            }
            finally {
                nonStandardServer.close();
            }
        ...
        it('should handle redirects with no target URL in error mode', async () => {
            ...
            const addr = noTargetServer.address();
