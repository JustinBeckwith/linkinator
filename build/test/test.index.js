...
describe('linkinator', () => {
    ...
    it('should skip links if asked nicely', async () => {
        ...
        assert.ok(results.passed);
        assert.strictEqual(results.links.filter((x) => x.state === LinkState.SKIPPED).length, 1);
    });
    it('should skip links if passed a linksToSkip function', async () => {
        ...
        assert.ok(results.passed);
        assert.strictEqual(results.links.filter((x) => x.state === LinkState.SKIPPED).length, 2);
    });
    it('should report broken links', async () => {
        ...
        assert.ok(!results.passed);
        assert.strictEqual(results.links.filter((x) => x.state === LinkState.BROKEN).length, 1);
    });
    it('should skip links with 999 status (bot-protected)', async () => {
        ...
        assert.ok(results.passed);
        assert.strictEqual(results.links.filter((x) => x.state === LinkState.SKIPPED).length, 1);
        const skippedLink = results.links.find((x) => x.state === LinkState.SKIPPED);
    ...
    it('should detect relative urls with absolute base', async () => {
        const mockPool = mockAgent.get('http://example.invalid');
        const fileContent = fs.readFileSync('test/fixtures/basetag/absolute.html', 'utf8');
        mockPool
            .intercept({ path: '/pageBase/index', method: 'GET' })
            .reply(200, fileContent, {
            headers: { 'content-type': 'text/html; charset=UTF-8' },
    ...
    it('should detect broken image links', async () => {
        ...
        assert.strictEqual(results.links.filter((x) => x.state === LinkState.BROKEN).length, 2);
        assert.strictEqual(results.links.filter((x) => x.state === LinkState.OK).length, 2);
    });
    ...
    it('should not follow non-http[s] links', async () => {
        ...
        assert.ok(results.passed);
        assert.strictEqual(results.links.filter((x) => x.state === LinkState.SKIPPED).length, 3);
    });
    ...
    it('should only follow links on the same origin domain', async () => {
        const mockPool = mockAgent.get('http://example.invalid');
        const fileContent = fs.readFileSync(path.resolve('test/fixtures/baseurl/index.html'), 'utf8');
        mockPool
            .intercept({ path: '/', method: 'GET' })
    ...
    it('should accept multiple http paths', async () => {
        const mockPool = mockAgent.get('http://example.invalid');
        const indexContent = fs.readFileSync('test/fixtures/local/index.html', 'utf8');
        const page2Content = fs.readFileSync('test/fixtures/local/page2.html', 'utf8');
    ...
    it('should respect globs', async () => {
        ...
        // reported for all parents to avoid result inflation)
        assert.strictEqual(results.links.length, 6);
        const licenseLinks = results.links.filter((x) => x.url.endsWith('LICENSE.md'));
    ...
    it('should autoscan mdx files in a repo scan', async () => {
        ...
        assert.ok(results.passed);
        assert.strictEqual(results.links.length, 3);
        assert.strictEqual(results.links.filter((x) => x.url.endsWith('guide.mdx')).length, 1);
        assert.strictEqual(results.links.filter((x) => x.url.endsWith('LICENSE.md')).length, 1);
    });
    ...
    it('should pass custom headers in requests', async () => {
        ...
        mockPool
            ...
            headers: (headers) => {
                ...
                return (headers['x-my-header'] === 'my-value' &&
    ...
    it('should merge custom headers with User-Agent when provided', async () => {
        ...
        mockPool
            ...
            headers: (headers) => {
                ...
                return (headers['user-agent']?.includes('Mozilla') &&
    ...
    it('should extract URLs from meta refresh tags', async () => {
        ...
        const metaRefreshLinks = results.links.filter((link) => link.url?.includes('example.invalid'));
    ...
    it('should extract URLs from inline CSS in style attributes and tags when checkCss is enabled', async () => {
        ...
        const inlineStyleUrls = results.links.filter((link) => link.url?.includes('example.invalid'));
        ...
        assert.ok(inlineStyleUrls.length >= 8, `Expected at least 8 URLs from inline styles, found ${inlineStyleUrls.length}`);
    });
    it('should NOT extract URLs from inline CSS when checkCss is disabled', async () => {
        ...
        const exampleInvalidUrls = results.links.filter((link) => link.url?.includes('example.invalid'));
        // Should only find the regular anchor link, not CSS URLs
        assert.strictEqual(exampleInvalidUrls.length, 1, `Expected only 1 URL (regular link), found ${exampleInvalidUrls.length}`);
    });
    ...
    it('should check alternate and canonical link tags', async () => {
        ...
        assert.strictEqual(results.links.length, 3);
        assert.strictEqual(results.links.filter((x) => x.state === LinkState.OK).length, 3);
    });
    it('should report broken links for all parent pages', async () => {
        ...
        const broken123Links = results.links.filter((x) => x.url === 'http://example.invalid/broken123');
        assert.strictEqual(broken123Links.length, 2);
        const parents = broken123Links.map((x) => x.parent).sort();
        assert.ok(parents[0]?.includes('pageA.html'), 'broken123 should be reported for pageA.html');
        assert.ok(parents[1]?.includes('pageB.html'), 'broken123 should be reported for pageB.html');
    });
    it('should resolve relative links correctly when URL has trailing slash', async () => {
        ...
        const apiKeysLink = results.links.find((x) => x.url.includes('manage/api-keys'));
        assert.ok(apiKeysLink, 'Should find the api-keys link resolved relative to trailing slash');
    });
    it('should resolve clean URLs when enabled', async () => {
        ...
        assert.ok(results.passed, 'All links should be valid with cleanUrls enabled');
        // Should find: index.html (root) + about + contact (relative paths)
        // Match flexibly for cross-platform compatibility
        const aboutLink = results.links.find((x) => x.url.includes('about') && !x.url.includes('about.html'));
        assert.ok(aboutLink, 'Should find the about link');
        assert.strictEqual(aboutLink?.state, LinkState.OK, 'about should resolve to about.html');
        const contactLink = results.links.find((x) => x.url.includes('contact') && !x.url.includes('contact.html'));
        assert.ok(contactLink, 'Should find the contact link');
        assert.strictEqual(contactLink?.state, LinkState.OK, 'contact should resolve to contact.html');
    });
    it('should NOT resolve clean URLs when disabled', async () => {
        ...
        const aboutLink = results.links.find((x) => x.url.includes('about') && !x.url.includes('about.html'));
        assert.ok(aboutLink, 'Should find the about link');
        assert.strictEqual(aboutLink?.state, LinkState.BROKEN, 'about should be broken without cleanUrls');
    });
    it('should resolve relative URLs without ./ prefix correctly (issue #374)', async () => {
        ...
        const harmonographLink = results.links.find((x) => x.url.includes('harmonograph'));
        assert.ok(harmonographLink, 'Should find the harmonograph link');
        assert.strictEqual(harmonographLink?.url, 'http://example.invalid/apps/web/harmonograph', 'harmonograph should resolve to /apps/web/harmonograph, not /apps/harmonograph');
        assert.strictEqual(harmonographLink?.state, LinkState.OK, 'harmonograph link should be OK');
        ...
        assert.strictEqual(otherLink?.url, 'http://example.invalid/apps/web/other', 'other should resolve to /apps/web/other');
        assert.strictEqual(otherLink?.state, LinkState.OK, 'other link should be OK');
    });
});
