...
describe('json-ld', () => {
    ...
    it('should find links in JSON-LD scripts', async () => {
        ...
        const brokenLinks = results.links.filter((link) => link.state === LinkState.BROKEN);
        assert.strictEqual(brokenLinks.length, 1);
        assert.strictEqual(brokenLinks[0].url, 'http://example.invalid/photos/16x9/photo.jpg');
