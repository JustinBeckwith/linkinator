...
describe('config', () => {
    ...
    it('should throw with a reasonable message if the path doesnt exist', async () => {
        ...
        await expect(getConfig(cfg)).rejects.toThrow(/ENOENT: no such file or directory/);
        await expect(getConfig({ config: '/path.dot/does/not/exist' })).rejects.toThrow(/ENOENT: no such file or directory/);
    });
    it('should throw a reasonable message if invalid extension is passed', async () => {
        await expect(getConfig({ config: 'invalid_extension.cfg' })).rejects.toThrow(/Config file should be either of extensions/);
    });
    it('should merge config settings from the CLI and object', async () => {
        ...
        const expected = JSON.parse(await fs.readFile(configPath, 'utf8'));
        expected.skip = 'loo';
    ...
    describe('json config file', () => {
        ...
        it('should throw with reasonable message if json file is in invalid format ', async () => {
            ...
            await expect(getConfig({ config: configPath })).rejects.toThrow(/Error parsing/);
        });
    });
    describe('js config file', () => {
        ...
        it('should import .js config file with absolute path', async () => {
            const configPath = path.resolve('test/fixtures/config/linkinator.config.js');
            const actualConfig = await getConfig({ config: configPath });
            const expectedConfig = {
                format: 'json',
                recurse: true,
    ...
    describe('.mjs config file', () => {
        ...
        it('should import .mjs config with absolute path', async () => {
            const configPath = path.resolve('test/fixtures/config/linkinator.config.mjs');
            const actualConfig = await getConfig({ config: configPath });
            const expectedConfig = {
                format: 'json',
                recurse: true,
    ...
    describe('.cjs config file', () => {
        ...
        it('should import cjs config with absolute path', async () => {
            const configPath = path.resolve('test/fixtures/config/linkinator.config.cjs');
